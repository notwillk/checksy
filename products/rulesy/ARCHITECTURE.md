...
lifecycle. The historical
[product-family decision](../../docs/decisions/rulesy-product-family.md) and
current [monorepo decision](../../docs/decisions/monorepo.md) define two
proposed adjacent products:
...
The [RulesyOS](../rulesyos/docs/design.md) and
[Rulesy Compose](../rulesy-compose/docs/design.md) designs are proposals, not
implemented subsystems. They are independently owned products within the
monorepo, with separate Cargo workspaces and releases when implemented. Their
provider, trust, state, image, publication, and OS concerns must not be
backported into this provisioner.
...
Moon `2.4.5` with Moon executor `2.4.5`, Rustup `1.29.0` with Rust `1.94.1`,
and Dev Container CLI `0.88.0`.
...
- The [compiled strict-configuration tests](tests/strict_configuration.rs)
...
- The compiled [process-runner contract tests](tests/process_runner_contract.rs)
...
- The compiled [P0 acceptance tests](tests/p0_acceptance.rs) map to the
...
- The network-free [helper tests](../../.devcontainer/scripts/tests/run.sh) cover
  version loading, supported architecture mapping, exact Moon release
...
- Manual fresh-container validation asserts Rulesy `0.8.1`, Entr availability,
  Moon `2.4.5`, Moon executor `2.4.5`, Rust `1.94.1` with `rustfmt` and
  `clippy`, Dev Container CLI `0.88.0`, Codex CLI `0.145.0` for local
  development, and Node.js 20 or newer.
