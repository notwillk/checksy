...
| State directory | Local ownership and complete verification metadata establish eligibility. | Lock mutations, stage on the same filesystem, and atomically select only completed generations. Protect and retain current and previous selections. | A private Linux/macOS generation-state substrate now validates protected roots, derives source/generation and whole-bundle digests, strictly opens completed payloads, atomically writes snapshots/records, leases live generations, and applies bounded retention. It is not wired to acquisition, `apply`, `status`, or the legacy commands, and atomic staging/promotion remains unimplemented. Legacy file-backed `install` and `check --fix` continue to lock only their mutable cache root. |
| Rule execution | Crossing this boundary executes arbitrary code. | Execute only an authenticated, strictly validated, locally authorized definition under its selected identity with bounded runtime. | On Linux/macOS, Bash commands run with `/dev/null` stdin, bounded output, and a 15-minute default timeout in a separate process group; an optional rule timeout may only shorten that limit. Commands still inherit the invoker's ambient identity and environment and are not sandboxed. |
...
The canonical-root advisory lock prevents cooperating file-backed `install` and
`check --fix` processes from mutating that legacy cache concurrently. It does not
cover ordinary checks or stdin fix mode, constrain an uncooperative local actor,
or replace descriptor-relative ancestor opening. The path-based controls
therefore remain raceable by an actor that can write the cache. Together these
controls reduce wrong-origin execution, structured path escape, and cooperating
mutation races, but do not make the mutable cache verified or constrain paths
deliberately accessed by arbitrary shell code.

Separately, the private generation-state module establishes protected state-root
and immutable-payload primitives. Its content marker is not a signer or
promotion receipt: authentication and selection events belong to the atomic
snapshot, allowing identical content to be authenticated again after local key
rotation. Because no command reaches this module yet, its integrity checks do
not make the current legacy cache or remote execution path trustworthy.

The current Linux/macOS subprocess runner also bounds Bash and Git work. It uses
15-minute check/fix budgets, 1-minute Git revision budgets, 5-minute Git
clone/fetch budgets, and a 5-second `TERM`-to-`KILL` grace period. It drains
stdout and stderr continuously, retaining at most 1 MiB of head/tail data per
stream, supplies `/dev/null` as stdin, disables standard Git/OpenSSH prompts,
and requests noninteractive credential helpers. A nonconforming helper can still
open `/dev/tty` until the source timeout. These controls do not cover the future
HTTP provider, limit CPU/memory/disk consumption, or forward signals received by
the Checksy parent. A background descendant that closes its capture descriptors
before the command leader exits, or one that deliberately creates another
process group or session, may outlive the invocation.
...
| Hung commands or credential prompts | Disable interactive source prompts; enforce timeouts and terminate process groups. | Timeouts are not a full process or resource sandbox; deliberately detached work or work not reached when the parent receives a signal may outlive Checksy. |
...
| Validation and bundle confinement | Runtime YAML rejects unknown fields, invalid scalar types, malformed rule forms, NUL bytes in constrained fields, invalid patterns, and invalid or excessive rule timeouts. Its generated Draft 7 schema has fixture-tested structural parity; duplicate YAML keys remain parser-owned, while complete Rust glob syntax and duration numeric-range/hard-maximum checks remain runtime-owned. The CLI retains origins for inline rules and per-config patterns, preflights patterns before commands, and confines fetched Git config/pattern targets to the canonical checkout. The public flat Rust loading/execution compatibility projection omits nested remote pattern groups because it cannot preserve their origins; local definitions retain legacy external-path behavior; HTTPS archive validation and protected local external-root policy are not implemented. | Schema-only consumers cannot detect the documented layered cases. A mutable or concurrently changed legacy checkout can invalidate path assumptions, trusted local definitions can select external content, archive attacks remain unhandled, and arbitrary shell commands can access paths outside every structured source boundary. |
| State integrity and concurrency | The private state module implements descriptor-relative protected-root opening, collision-resistant IDs, whole-tree digests, strict state/marker records, atomic JSON replacement, generation leases, and bounded generation/failure/audit cleanup. It remains unwired: file-backed `install` and `check --fix` still use the legacy cache-root lock, ordinary `check` and stdin fix mode remain unlocked, and there is no staged acquisition or atomic promotion path selecting current/previous for commands. | The substrate can reject corrupt test/state inputs, but current command execution still races and trusts the mutable legacy cache. Interrupted acquisition, provider authentication, and last-known-good selection are not yet protected end to end. |
| Process and resource bounds | Linux/macOS checks, fixes, rechecks, pattern scripts, and Git operations now share bounded subprocess execution. Bash defaults to 15 minutes, Git resolution to 1 minute, and Git clone/fetch to 5 minutes; a positive rule timeout can only shorten the Bash default. Each child receives `/dev/null` stdin in its own process group, standard Git/OpenSSH prompts are disabled, credential helpers are requested to be noninteractive, timeout cleanup escalates after 5 seconds, and each output stream is capped at 1 MiB of head/tail data. HTTP acquisition bounds remain unimplemented, as do parent-signal forwarding and OS resource quotas. | A nonconforming credential helper can still open `/dev/tty` until timeout. Background descendants that close capture descriptors before ordinary leader exit and deliberately detached/session-changing descendants may outlive Checksy; an external signal to Checksy may leave work behind; and authorized commands can still exhaust CPU, memory, disk, or network resources within host limits. |
