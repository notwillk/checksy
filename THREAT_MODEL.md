...
| State directory | Local ownership and complete verification metadata establish eligibility. | Lock mutations, stage on the same filesystem, and atomically select only completed generations. Protect and retain current and previous selections. | A reusable advisory-lock primitive now serializes complete file-backed `install` and `check --fix` runs on their canonical legacy cache root. There is still no protected generation state store, atomic promotion, or current/previous selection; ordinary checks and stdin fix mode are unlocked. |
...
| State integrity and concurrency | File-backed `install` and the complete `check --fix` path now use an operating-system advisory lock on the canonical legacy cache root. Ordinary `check` and stdin fix mode remain unlocked. There is no protected generation state store, durable status/audit metadata, descriptor-relative ancestor handling, or atomic current/previous selection. | Cooperating file-backed mutations are serialized, but an uncooperative local actor or an unlocked reader can still race the mutable cache. Local tampering and interrupted updates still leave no trustworthy selection history. |
