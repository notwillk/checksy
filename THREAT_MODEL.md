...
| Validation and bundle confinement | Runtime YAML rejects unknown fields, invalid scalar types, malformed rule forms, and invalid patterns, but the emitted schema is not yet generated from those types. Fetched paths are not confined to a verified bundle, and remote definitions lose some origin information during expansion. | Schema drift, path traversal, archive/link escape, and incorrect asset origin are not covered by strict end-to-end validation. |
